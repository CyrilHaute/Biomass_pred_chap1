...
glm_function <- function(biomass = biomass, 
  ...
  predictions <- pbmclapply(1:length(biomass), function(i){
    ...
    covNames_new_2 <- covNames_new_2[-which(covNames_new_2 %in% c('I(factor(cov21)^2)'))]
    ...
    species_j <- mclapply(2:length(raw_biomass$fitting), function(j){
      ...
      if(length(unique(biomass_final$cov21)) == 1){
        ...
        model_fit <- tryCatch(glm(formula = form2, family = gaussian, data = biomass_final), error = function(e) NA)
          ...
          test <- unique(biomass_only_val$cov21) %in% unique(biomass_final$cov21) # test if you have the same MPA effectivness factors in the fitting and validation set
      ...
      predictions}else{predictions <- NA}
